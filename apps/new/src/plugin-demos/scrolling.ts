...
export class DemoModel extends DemoSharedBase {
  ...
  constructor(container: View) {
    ...
    this.automatedProfiling = getViewById(
      container,
      'automated-profiling'
    ) as Button;
  ...
  toggleManualProfiling({ value }: PropertyChangeData): void {
    console.log(
      `toggleManualProfiling changed to ${value}. manualProfiling:`,
      this.manualProfiling
    );
  ...
  runAutomatedProfiling(): void {
    ...
    console.log(
      `runAutomatedProfiling. automatedProfiling:`,
      this.automatedProfiling
    );
  ...
  private updateAutomatedProfilingEnabled(): void {
    this.automatedProfiling.isEnabled =
      !this.automatedProfilingInProgress && !this.manualProfiling.checked;
  }
}
